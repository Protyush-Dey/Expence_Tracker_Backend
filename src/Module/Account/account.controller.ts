...
class AccountController extends BaseController {
  ...
  // get all account and their balance
  getAllAccountDetails = asyncHandler(async (req: Request, res: Response) => {
    const accounts = await accountService.getAllAccountDetails(
      this.getUserId(req)
    );
    return res
      .status(200)
      .json(new ApiResponse(200, "All account data", accounts));
  });

  // get expense with current month
  getMonthExpenseOfAccount = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      return res
        .status(200)
        .json(new ApiResponse(200, "Monthly expenses fetched", data));
    }
  );

  // get expense with dates
  getExpenseOfAccountByDates = asyncHandler(
    async (req: Request, res: Response) => {
      const { accountNo } = req.params;
